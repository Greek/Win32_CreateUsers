...
def main(args):
    with open('./userpass.csv') as csvfile:
        ...
        group = None
        if args.mode == "domain":   
            group = f"{args.group}.lab.tu"
           
        if args.mode == "domain" and args.group is None:
            raise Exception("You must provide a group name with domain mode")
        ...
        for row in userpassreader:
            ...
            try:
                win32net.NetUserAdd(group, 1, dict (
                    name = row["username"],
                ...
                print(f"Created user {row["username"]}, with password {row["password"]} to {args.mode} users")
                count += 1
